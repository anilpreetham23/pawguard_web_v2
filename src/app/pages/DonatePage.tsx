...
import { PageShell, Section, Button, Input, Card, Reveal, DonationTransparencyLedger } from "../components/pawguard";
import { DonationActionPanel } from "../components/pawguard/DonationActionPanel";
import { useDonationState, PRESET_AMOUNTS, DONATION_TIERS } from "../hooks/useDonationState";
            ...
            <div className="max-w-[600px] mx-auto">
              <Card variant="elevated" role="alert">
                ...
                <div className="flex flex-col gap-2">
                  <h2 className="text-foreground font-bold text-xl">Payment declined</h2>
                  <p className="text-muted-foreground text-sm leading-relaxed">
                    Your donation could not be processed. This may be due to insufficient funds or a temporary issue with your payment method.
                  </p>
                </div>
            ...
            <div className="max-w-[600px] mx-auto">
              <Card variant="elevated" role="status" aria-live="polite">
                ...
                <div className="flex flex-col gap-3">
                  <h2 className="text-foreground font-bold text-2xl">Thank You</h2>
                  <p className="text-muted-foreground text-base leading-relaxed">
                    Your {frequency === "monthly" ? "monthly" : "one-time"} gift of{" "}
                    <strong>${displayAmount?.toFixed(2)}</strong> has been received. You will receive a tax receipt by email within 24 hours.
                  </p>
                  <div className="bg-primary/5 border border-primary/10 rounded-xl p-4 flex items-center gap-4">
                    <span className="font-serif text-primary font-bold text-3xl">
                      {displayAmount && displayAmount >= 500 ? "1" : displayAmount && displayAmount >= 250 ? "1" : displayAmount && displayAmount >= 100 ? "1" : displayAmount && displayAmount >= 50 ? "1" : "7"}
                    </span>
                    <div className="flex flex-col">
                      <span className="text-foreground font-semibold text-sm">Your direct impact</span>
            ...
            <form onSubmit={handleSubmit}>
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-[var(--space-12)] lg:gap-[var(--space-16)]">
                <div className="lg:col-span-7 order-2 flex flex-col gap-8">
                  <div className="flex flex-col gap-3">
                    <label className="text-foreground text-xs font-semibold tracking-wider uppercase font-condensed">Giving Frequency</label>
                    <div className="grid grid-cols-2 gap-3">
                      <button
                        type="button"
                        onClick={() => setFrequency("monthly")}
                        className={`py-4 text-center font-semibold text-sm tracking-wider border-2 rounded-btn transition-all duration-ui ${
                          frequency === "monthly"
                  ...
                  <div className="flex flex-col gap-3">
                    <label className="text-foreground text-xs font-semibold tracking-wider uppercase font-condensed">Select Amount</label>
                    <div className="grid grid-cols-3 sm:grid-cols-5 gap-3">
                      {PRESET_AMOUNTS.map((amt) => (
                        <button
                          ...
                          className={`py-3.5 text-center font-bold text-base border-2 rounded-btn transition-all duration-ui ${
                    ...
                    <div className="flex items-center gap-3 mt-1">
                      <label htmlFor="custom-amount" className="text-muted-foreground text-sm shrink-0">Custom amount:</label>
                  ...
                  <div className="flex flex-col gap-5">
                    <label className="text-foreground text-xs font-semibold tracking-wider uppercase font-condensed">Payment Details</label>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <Input label="First Name" placeholder="Jane" autoComplete="given-name" />
                    <Input label="Last Name" placeholder="Smith" autoComplete="family-name" />
                  </div>
                    <Input label="Email Address" type="email"                         placeholder="your@email.com" autoComplete="email" inputMode="email" />

                    <div className="border-2 border-dashed border-border rounded-card p-6 text-center flex flex-col items-center gap-3 bg-card">
                      <Lock size={18} className="text-muted-foreground" />
                      <div>
                        <p className="text-foreground font-semibold text-sm">Demo Mode</p>
                        <p className="text-muted-foreground text-xs mt-1">No real payment is processed. This is a demonstration interface.</p>
                      </div>
                    </div>
                  </div>
                ...
                <div className="lg:col-span-5 order-1 flex flex-col gap-8">
                  <div className="flex flex-col gap-5">
                    <h2 className="text-foreground font-bold text-xl">Your Impact</h2>
                    <div className="grid grid-cols-3 sm:grid-cols-5 gap-2.5">
                      {DONATION_TIERS.map((item) => {
                        const Icon = item.icon;
                        const isActive = item === activeTier;
                        const isPast = DONATION_TIERS.indexOf(item) < DONATION_TIERS.indexOf(activeTier);
                        return (
                          <div
                            key={item.amount}
                          ...
                          >
                            ...
                            <div
                              className={`absolute -top-1.5 -right-1.5 w-5 h-5 rounded-full flex items-center justify-center transition-all duration-gentle ${
                                isActive ? "bg-emergency scale-100" : "scale-0"
                              }`}
                            >
                    ...
                    <div className="bg-card border border-border rounded-card p-5 shadow-sm min-h-[80px] flex items-center transition-all duration-gentle">
                      {displayAmount ? (
                        <div className="flex items-start gap-4">
                          <div className={`w-12 h-12 rounded-xl flex items-center justify-center shrink-0 ${activeTier.bg}`}>
                            <activeTier.icon size={22} className={activeTier.color} />
                          </div>
                          <div>
                            <p className="text-foreground font-semibold text-base">
                              Your <span className="font-mono tabular-nums">${displayAmount}</span> {frequency === "monthly" ? "monthly " : ""}gift
                            </p>
                        ...
                        <p className="text-muted-foreground text-sm">Select an amount to see your impact</p>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="bg-card border border-border rounded-card p-6 shadow-sm">
                    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" className="text-primary/30 mb-3">
                      <path d="M3 21c3 0 7-1 7-8V5c0-1.25-.756-2.017-2-2H4c-1.25 0-2 .75-2 1.972V11c0 1.25.75 2 2 2 1 0 1 0 1 1v1c0 1-1 2-2 2s-1 .008-1 1.031V20c0 1 0 1 1 1z" />
                      <path d="M15 21c3 0 7-1 7-8V5c0-1.25-.757-2.017-2-2h-4c-1.25 0-2 .75-2 1.972V11c0 1.25.75 2 2 2h.75c0 2.25.25 4-2.75 4v3c0 1 0 1 1 1z" />
                    </svg>
                    <p className="text-foreground text-sm italic leading-relaxed mb-3">
                      &ldquo;I donate monthly because I know exactly where my money goes. 78 cents of every dollar hits the ground running. That is transparency I can trust.&rdquo;
                    </p>
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex flex-col">
                        <span className="text-foreground font-semibold text-xs">Amara K.</span>
                        <span className="text-muted-foreground text-2xs">Monthly donor since 2023</span>
                      </div>
                    </div>
                  </div>
                </div>
